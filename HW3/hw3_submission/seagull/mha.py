...
class MultiHeadAttention(Module):
    def __init__(
        ...
        """
        Initializes the multihead attention that allows the model to jointly attend to information
    ...
    def masked_attention_probs(
        ...
        Parameters
        ----------
        query: torch.Tensor (batch_size, num_heads, max_length, head_dim)
            ...
            for more details.
        key: torch.Tensor (batch_size, num_heads, max_length, head_dim)
        ...
        import math
        # TODO-4.1
        # 1. compute similarities as QK^T/sqrt(dk)
        batch_size, num_heads, max_length, head_dim = query.shape
        # b=batch_size, h=num_heads, l=length of query, m=length of key, d=head_dim
        similarities = torch.einsum("bhld,bhmd->bhlm", query, key)/math.sqrt(head_dim)

        # 2. set the attention scores with pad tokens a large negative value
        # 2.1 transform the dimension of padding mask from (batch_size, max_length) to (batch_size, 1, 1, max_length)
        if padding_mask is not None:
          padding_mask = padding_mask.unsqueeze(1).unsqueeze(2)
          # 2.2 set the large negative value
          similarities = similarities.masked_fill(mask = padding_mask, value=float('-inf'))

        # 3. mask out the upper-triangular part
        # shape: (batch_size, num_heads, length of query, length of key)
        masked_similarities = self._apply_causal_mask(similarities)

        # 4. do the softmax along the keys dimension
        masked_attn_probs = softmax(y=masked_similarities, dim=-1, dropout_proba=self.attn_dropout_proba, training=self.training)
        return masked_attn_probs

    @staticmethod
    def attention(masked_attn_probs: torch.Tensor, value: torch.Tensor) -> torch.Tensor:
        # similarities: (batch_size, num_heads, max_length, max_length)
        # value: (batch_size, num_heads, max_length, head_dim)
        # TODO-4.2
        # b=batch_size, h=num_heads, l=max_length, m=max_length
        # b=batch_size, h=num_heads, l=max_length, d=head_dim
        # return: (batch_size, num_heads, max_length, head_dim), which is (b,h,l,d)
        attention_scores = torch.einsum("bhlm,bhmd->bhld", masked_attn_probs, value)
    ...
    def forward(
        ...
        padding_mask: Optional[torch.Tensor]
            If specified, a mask indicating which elements within key to ignore
