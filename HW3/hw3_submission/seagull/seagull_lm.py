...
class SeagullLM(Module):
    ...
    def forward(
        ...
        if return_attentions:
            hidden_states, attentions = outputs
        else:
        ...
        if self.weight_tying:
            lm_logits = final_hidden @ self.seagull.embedding.token_embedding.weight.T
        else:
    ...
    def talk(
        ...
        ----------
        input_ids : torch.Tensor
            The input IDs to feed as input to the language model.
        max_new_tokens : int
            The maximum number of tokens to generate.
        temperature : float
            The temperature value used to control generation (higher = diverse).
        num_samples : int
            The number of samples to generate.
        top_k : Optional[int]
            The number of top candidates to use in generating samples.
        ...
        -------
        List[List[int]]
            Output list of IDs corresponding to output text generated.
        """
