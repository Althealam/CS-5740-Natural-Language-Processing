...
from masking.data_processing.constants import PAD_MASK_TAG, PAD_TOKEN, UNK_TOKEN, MASK_ENCODING_MAP
from masking.data_processing.data_collator import DataCollator
from masking.data_processing.tokenizer import Tokenizer
from masking.models import MaskPredictor
from masking.trainers.trainer import Trainer
from masking.utils.utils import set_seed
...
def _run_model_and_write_preds(
    ...
    model = MaskPredictor(
        ...
        output_dim=len(MASK_ENCODING_MAP) - 1,
        **config["model"],
    )
    model.print_params()
...
def _write_mask_files(net_ids: Optional[str], mask_outputs_path: str, is_milestone_submission: bool) -> None:
    mask_basepath = "masking"
    files_to_copy = [
        os.path.join(mask_basepath, "data_processing/tokenizer.py"),
        os.path.join(mask_basepath, "data_processing/data_collator.py"),
        os.path.join(mask_basepath, "nn/embeddings/embedding.py"),
        os.path.join(mask_basepath, "trainers/trainer.py"),
    ]
    if not is_milestone_submission:
        files_to_copy = files_to_copy + [
            os.path.join(mask_basepath, "nn/models/ffnn.py"),
            os.path.join(mask_basepath, "nn/models/rnn.py"),
        ]
    for file in files_to_copy:
        shutil.copy2(file, mask_outputs_path)
        if net_ids is not None:
            _append_net_ids_to_file(filename=os.path.join(mask_outputs_path, os.path.basename(file)), net_ids=net_ids)


def _make_output_dirs(
    basepath_to_store_submission: str, is_leaderboard_submission: bool, is_milestone_submission: bool
) -> Dict[str, str]:
    mask_outputs_path = os.path.join(basepath_to_store_submission, "masking")
    ...
    if not is_leaderboard_submission:
        os.makedirs(mask_outputs_path, exist_ok=True)
        if not is_milestone_submission:
    ...
    return {
        ...
        "mask_outputs_path": mask_outputs_path,
...
def main(
    ...
    if not is_leaderboard_submission:
        ...
        _write_mask_files(
            mask_outputs_path=all_output_paths["mask_outputs_path"],
            is_milestone_submission=is_milestone_submission,
            net_ids=net_ids,
        )
    ...
    if not is_milestone_submission:
        ...
        data_collator = DataCollator(tokenizer=tokenizer, pad_tag=PAD_MASK_TAG, **config["data_collator"])
        if not is_leaderboard_submission:
...
def argparser():
    ...
    parser.add_argument(
        "--ffnn-config-path",
        type=str,
        help="Path to the ffnn-based mask predictor config file (from the artefacts).",
        default=None,
    )
    parser.add_argument(
        "--rnn-config-path",
        type=str,
        help="Path to the rnn-based mask predictor config file (from the artefacts).",
        default=None,
    )
    parser.add_argument(
